...
public static class RegularExpressions
{
    ...
    {
        if (ignoreCase) searchTerm = searchTerm.ToLower();
        ...
        foreach (var item in items)
        {
            ...
            if (ignoreCase) propertyValue = propertyValue.ToString()?.ToLower();

            if (propertyValue!.ToString()!.Contains(searchTerm)) filtered.Add(item);
        }
    ...
    {
        if (!NetTools.Common.Validation.Exists(text)) return false;
    ...
    /// </summary>
    /// <param name="text">String to check.</param>
    /// <param name="regularExpression">Pattern to match against.</param>
    /// <param name="ignoreCase">Whether to ignore case when evaluating string.</param>
    /// <returns>True if the string matches the pattern, false otherwise.</returns>
    public static bool Matches(string text, RegularExpression regularExpression, bool ignoreCase = true)
    {
        return Matches(text, regularExpression.Pattern, ignoreCase);
    }
    ...
    {
        if (!NetTools.Common.Validation.Exists(text)) return string.Empty; // text is null or empty, return empty string
    ...
    /// </summary>
    /// <param name="text">Text to search within.</param>
    /// <param name="regularExpression">Pattern to search for.</param>
    /// <param name="ignoreCase">Whether to ignore case when evaluating string.</param>
    /// <param name="replacement">String to replace matches with.</param>
    /// <returns></returns>
    public static string Replace(string text, RegularExpression regularExpression, string replacement, bool ignoreCase = true)
    {
        return Replace(text, regularExpression.Pattern, replacement, ignoreCase);
    }
    ...
    /// <param name="ignoreCase">Whether to ignore case when evaluating string.</param>
    /// <returns>True if a matching substring exists, false otherwise.</returns>
    public static bool SubstringExists(string text, RegularExpression regularExpression, bool ignoreCase = true)
    {
        return SubstringExists(text, regularExpression.Pattern, ignoreCase);
    }
    ...
    public static List<string> Substrings(string text, string pattern, bool ignoreCase = true)
    {
        ...
        if (!NetTools.Common.Validation.Exists(text)) return substrings; // empty list

        var rx = new Regex(pattern, GetDefaultOptions(ignoreCase));
        var matches = rx.Matches(text);
        foreach (Match match in matches) substrings.Add(match.Value);
    ...
    /// <param name="ignoreCase">Whether to ignore case when evaluating string.</param>
    /// <returns></returns>
    public static List<string> Substrings(string text, RegularExpression regularExpression, bool ignoreCase = true)
    {
        return Substrings(text, regularExpression.Pattern, ignoreCase);
    }
