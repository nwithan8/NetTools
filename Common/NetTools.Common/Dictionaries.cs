...
public static class Dictionaries
{
    ...
    public static Dictionary<string, object> ConvertToStringNonNullableObjectDictionary(
    ...
    /// <param name="dictionary">A <see cref="Dictionary{TKey,TValue}"/> to convert.</param>
    /// <returns>A <see cref="Dictionary{TKey,TValue}"/> of string, object? pairs.</returns>
    public static Dictionary<string, object?> ConvertToStringNullableObjectDictionary(
        Dictionary<string, object> dictionary)
    {
        var newDictionary = new Dictionary<string, object?>();
        foreach (var item in dictionary)
    ...
    internal static Dictionary<string, object> Wrap(Dictionary<string, object> dictionary, params string[] keys) => keys
        .Reverse().Aggregate(dictionary, (current, key) => new Dictionary<string, object> { { key, current } });
    ...
    /// <returns>A T-type object, or default if key does not exist.</returns>
    internal static T? GetOrNullEnum<T>(Dictionary<string, object> dictionary, string key) where T : ValueEnum
    {
        if (!dictionary.TryGetValue(key, out var value)) return null;
        return value switch
        {
